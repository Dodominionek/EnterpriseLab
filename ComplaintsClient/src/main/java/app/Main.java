...
public class Main {
    ...
    public static void main(String[] args){
        ...
        Complaint[] complaints = client.target("http://localhost:8080/Complaints/" + "resources/complaints")
                ...
                .get(Complaint[].class);
        System.out.println("Complaints before update: \n" + Arrays.toString(complaints));
        
        Complaint complaint = null;
        for (Complaint com : complaints) {
            if (com.getStatus().equals("open")) {
                complaint = client.target("http://localhost:8080/Complaints/" + "resources/complaints/" + com.getId())
                        .request(MediaType.APPLICATION_JSON)
                        .get(Complaint.class);
                break;
            }
        ...
        Response response = client.target("http://localhost:8080/Complaints/" + "resources/complaints/" + complaint.getId())
                .request()
                .put(Entity.entity(complaint, MediaType.APPLICATION_JSON));
        System.out.println("Status:\n"+ response.getStatus());
        
        Complaint[] openedComplaintList = client.target("http://localhost:8080/Complaints/" + "resources/complaints")
            .queryParam("status", "open")
            .request(MediaType.APPLICATION_JSON)
            .get(Complaint[].class);
        System.out.println("Open complaints:\n" + Arrays.toString(openedComplaintList));
